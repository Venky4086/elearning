...
@Component({
  selector: 'app-qbankedit',
  templateUrl: './qbankedit.component.html',
  styleUrls: ['./qbankedit.component.css']
})
export class QbankeditComponent implements OnInit {
...
getallqbankmode(){
  this.spinner.show();
  this.qbankservice.single_data_qbank(this.qbank_id).subscribe((res)=>{
    console.log(res);
    if(res){
      this.qbankmodes = res
...
// add qubank mode
  ...
  editqbank(){
    ...
    if(this.EditQbank.invalid){
      ...
      this.qbankservice.update_qbank(this.qbank_id,formData).subscribe((res)=>{
        console.log(res);
        this.toastr.success(this.message,'Sucessfully update the qbankmode', {
          positionClass: 'toast-top-center'
        });
