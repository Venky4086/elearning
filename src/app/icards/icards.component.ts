...
@Component({
  selector: 'app-icards',
  templateUrl: './icards.component.html',
  styleUrls: ['./icards.component.css']
})
...
// all icards
  ...
  all_list_icards(){
    this.spinner.show();
    this.elearningservice.icard_list().subscribe((res)=>{
      if(res){
        if(res == 0){
          this.spinner.hide();
          console.log(res);
...
// add icards
  ...
  add_icards(){
    ...
    if(this.Icards.invalid){
      ...
      this.elearningservice.add_icard(formData).subscribe((res)=>{
...
// edit icards
  ...
  updateicarde(){ 
   ...
   if(this.UpdateIcards.invalid){
    ...
    formData.append('pdf', this.file);
    console.log(formData)
    this.elearningservice.update_icard(this.icards_id,formData).subscribe((res)=>{
