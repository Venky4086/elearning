...
@Component({
  selector: 'app-qbanksubcategory',
  templateUrl: './qbanksubcategory.component.html',
  styleUrls: ['./qbanksubcategory.component.css']
})
export class QbanksubcategoryComponent implements OnInit {
  submitted=false;
  message:any;
  category_name:any;
  ...
  subcategory_id: any;
  no_data = false;
  constructor(private qubankservice:QBankserviceService,private spinner: NgxSpinnerService,private fb:FormBuilder,private toastr: ToastrService,private elearningservice:ElearningService) {
   }
  ...
  getallcategorys(){
    // this.spinner.show();
    this.qubankservice.qbank_category_list().subscribe((res)=>{
      if(res){
        // this.spinner.hide();
        console.log(res);
  ...
  getallsubcategory(){
    this.spinner.show();
    this.qubankservice.qbank_subcategory_list().subscribe((res)=>{
     if(res){
        if(res == 0){
          this.spinner.hide();
          console.log(res);
...
// add subcategories
  ...
  addsubcategory(){
    ...
    if(this.Addsubcategory.invalid){
      ...
      console.log(data);
      this.qubankservice.add_qbank_subcategory(data).subscribe((res)=>{
        if(res){
        console.log(res);
  ...
  updatesubcategory(){
    ...
    if(this.Updatesubcategory.invalid){
      ...
      console.log(data);
      this.qubankservice.update_qbank_subcategory(this.subcategory_id,data).subscribe((res)=>{
        if(res){
        console.log(res);
  ...
  deletesubcategory(){
    this.qubankservice.delete_qbank_subcategory(this.subcategory_id).subscribe((res)=>{
      console.log(res);
